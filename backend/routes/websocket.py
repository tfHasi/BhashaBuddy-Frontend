...
async def websocket_score_updates(websocket: WebSocket):
    await ConnectionManager.connect_score_updates(websocket)
    try:
        while True:
            # Keep connection alive
            await websocket.receive_text()
    except WebSocketDisconnect:
        ConnectionManager.disconnect_score_updates(websocket)
...
async def websocket_leaderboard(websocket: WebSocket):
    await ConnectionManager.connect_leaderboard(websocket)
    try:
        while True:
            # Keep connection alive
            await websocket.receive_text()
    except WebSocketDisconnect:
        ConnectionManager.disconnect_leaderboard(websocket)
